...
/**
  * Clase que permite crear listas simplemente ligadas ordenadas.
  * <br>Se extienden las capacidades de la clase Lista
...
public class ListaOrdenada<T> extends Lista<T>{
  protected final Comparator<T> prueba;
  ...
  /**
   * Inserta el primer elemento de la lista. Se sobreescribe la implementación de Lista
  ...
  public void agregar(T dato){
    if (estaVacia())
      ...
      if (prueba.compare(dato,inicio.elemento) <= 0) //Si el elemento que llega es menor
        inicio = new Nodo<T>(dato,inicio);
      else{
        Nodo<T> pos = inicio.siguiente;
        Nodo<T> ant = inicio;
